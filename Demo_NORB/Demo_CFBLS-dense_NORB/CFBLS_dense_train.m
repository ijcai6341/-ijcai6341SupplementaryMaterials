function [TrainingAccuracy,TestingAccuracy,Training_time,Testing_time] = CFBLS_dense_train(train_x,train_y,test_x,test_y,s,C,N1,N2,N3)
...
H1 = [train_x .1 * ones(size(train_x,1),1)];y=zeros(size(train_x,1),N2*N1*2);
for i=1:N2
    we=2*rand(size(H1,2),N1)-1;
    We{i}=we;
    A1 = H1 * we;A1 = mapminmax(A1);
    clear we;
beta1  =  sparse_bls(A1,H1,1e-3,70)';
beta11{i}=beta1;
% clear A1;
T1 = H1 * beta1;
...
%clear test_x;
yy1=zeros(size(test_x,1),N2*N1*2);
for i=1:N2
