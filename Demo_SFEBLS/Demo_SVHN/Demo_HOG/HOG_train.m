function [TrainingAccuracy,TestingAccuracy,Training_time,Testing_time] = HOG_train(train_x,train_y,test_x,test_y,s,C,N1,N2,N3)
...
for i=1:size(train_x,1)
    y1=[];
    y1=cat(1,y1,extractHOGFeatures(reshape(train_x(i,1:1024),32,32)/255,'CellSize',[4 4])');
    y1=cat(1,y1,extractHOGFeatures(reshape(train_x(i,1025:2048),32,32)/255,'CellSize',[4 4])');
    y1=cat(1,y1,extractHOGFeatures(reshape(train_x(i,2049:end),32,32)/255,'CellSize',[4 4])');
    y=[y,y1];
    clear y1;
end
train_x=y';
...
for i=1:size(test_x,1)
    y1=[];
    y1=cat(1,y1,extractHOGFeatures(reshape(test_x(i,1:1024),32,32)/255,'CellSize',[4 4])');
    y1=cat(1,y1,extractHOGFeatures(reshape(test_x(i,1025:2048),32,32)/255,'CellSize',[4 4])');
    y1=cat(1,y1,extractHOGFeatures(reshape(test_x(i,2049:end),32,32)/255,'CellSize',[4 4])');
    y=[y,y1];
    clear y1;
end
test_x=y';
